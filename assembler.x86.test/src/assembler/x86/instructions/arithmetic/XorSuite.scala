  ...
  "an Xor instruction" when {
    "in real mode" should {
      ...
      "correctly encode xor al, 0x40" in {
        Xor(0x40.toByte, AL).encode should be(Hex("34 40"))
      }

      "correctly encode xor ax, 0x3412" in {
        Xor(0x3412.toShort, AX).encode should be(Hex("35 12 34"))
      }
      ...
      "correctly encode xor al, al" in {
        Xor(AL, AL).encode should be(Hex("30 C0"))
      }
      ...
      "correctly encode xor ax, ax" in {
        Xor(AX, AX).encode should be(Hex("31 C0"))
      }

      "correctly encode xor eax, 0x44332211" in {
        Xor(0x44332211, EAX).encode should be(Hex("66 35 11 22 33 44"))
      }

      "correctly encode xor bl, 0x40" in {
        Xor(0x40.toByte, BL).encode should be(Hex("80 F3 40"))
      }

      "correctly encode xor [bx], 0x3412" in {
        Xor(0x3412.toShort, RegisterMemoryLocation.wordSize(BX)).encode should be(Hex("81 37 12 34"))
      }

      "correctly encode xor [bx], al" in {
        Xor(AL, RegisterMemoryLocation(BX)).encode should be(Hex("30 07"))
      }

      "correctly encode xor ah, [si]" in {
        Xor(RegisterMemoryLocation(SI), AH).encode should be(Hex("32 24"))
      }
    }
    ...
    "in protected mode" should {
      ...
      "correctly encode xor DWORD PTR [0x11111111], 0x44332211" in {
        Xor(0x44332211, MemoryAddress.doubleWordSize(0x11111111.encodeLittleEndian)).encode should be(Hex("81 35 11 11 11 11 11 22 33 44"))
      }

      "correctly encode xor WORD PTR [0x11111111], 0x44" in {
        Xor(0x44.toByte, MemoryAddress.wordSize(0x11111111.encodeLittleEndian)).encode should be(Hex("66 83 35 11 11 11 11 44"))
      }

      "correctly encode xor [0x1234], edx" in {
        Xor(EDX, MemoryAddress(0X1234.toShort.encodeLittleEndian)).encode should be(Hex("67 31 16 34 12"))
      }

      "correctly encode xor esi, gs:[eax+0x12]" in {
        Xor(RegisterMemoryLocation.withSegmentOverride(EAX, 0x12.toByte.encodeLittleEndian, GS), ESI).encode should be(Hex("65 33 70 12"))
      }
    }
    ...
    "in long mode" should {
      ...
      "correctly encode xor rax, 0x78776655" in {
        Xor(0x78776655, RAX).encode should be(Hex("48 35 55 66 77 78"))
      }
      ...
      "correctly encode xor rax, 0x88776655" in {
        Xor(0x88776655, RAX).encode should be(Hex("48 35 55 66 77 88"))
      }
      ...
      "correctly encode xor DWORD PTR [rax+rbx*2], 0x44332211" in {
        Xor(0x44332211, SIBMemoryLocation.doubleWordSize(RBX, RAX, scale = 2)).encode should be(Hex("81 34 58 11 22 33 44"))
      }

      "correctly encode xor rbx, 0x44332211" in {
        Xor(0x44332211, RBX).encode should be(Hex("48 81 F3 11 22 33 44"))
      }

      "correctly encode xor DWORD PTR [rax+rbx*2], 0x44" in {
        Xor(0x44.toByte, SIBMemoryLocation.doubleWordSize(RBX, RAX, scale = 2)).encode should be(Hex("83 34 58 44"))
      }

      "correctly encode xor [rbx], al" in {
        Xor(AL, RegisterMemoryLocation(RBX)).encode should be(Hex("30 03"))
      }

      "correctly encode xor [rbx], ah" in {
        Xor(AH, RegisterMemoryLocation(RBX)).encode should be(Hex("30 23"))
      }

      "correctly encode xor [rbx], r15b" in {
        Xor(R15L, RegisterMemoryLocation(RBX)).encode should be(Hex("44 30 3B"))
      }

      "correctly encode xor [rax], rax" in {
        Xor(RAX, RegisterMemoryLocation(RAX)).encode should be(Hex("48 31 00"))
      }

      "correctly encode xor r15, [r11]" in {
        Xor(RegisterMemoryLocation(R11), R15).encode should be(Hex("4D 33 3B"))
      }
    }
  }
}