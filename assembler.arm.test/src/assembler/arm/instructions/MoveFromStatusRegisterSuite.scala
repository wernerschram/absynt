  ...
  "an MoveFromStatusRegister instruction" when {
    "in a32 mode" should {
      ...
      "correctly encode mrs r1, CPSR" in {
        MoveFromStatusRegister(CPSR, R1).encode should be(Hex.MSB("e10f1000"))
      }

      "correctly encode mrscc r1, SPSR" in {
        MoveFromStatusRegister(SPSR, R1, Condition.UnsignedLower).encode should be(Hex.MSB("314f1000"))
      }
    }
  }
  ...
  "an MoveToStatusRegister instruction" when {
    "in a32 mode" should {
      ...
      "correctly encode msr SPSR_xc, r1" in {
        MoveToStatusRegister(R1, SPSR, Fields.control + Fields.extension).encode should be(Hex.MSB("e163f001"))
      }

      "correctly encode msr CPSR_fsxc, r9" in {
        MoveToStatusRegister(R9, CPSR, Fields.extension + Fields.control + Fields.status + Fields.flags).encode should be(Hex.MSB("e12ff009"))
      }

      "correctly encode msr CPSR_fx, #240, 4" in {
        MoveToStatusRegister(0xf0.toByte, 4.toByte, CPSR, Fields.extension + Fields.flags).encode should be(Hex.MSB("e32af2f0"))
      }

      "correctly encode msr CPSR_fsxc, #16777216" in {
        MoveToStatusRegister(1.toByte, 8.toByte, CPSR, Fields.extension + Fields.control + Fields.status + Fields.flags).encode should be(Hex.MSB("e32ff401"))
      }
      
      "correctly encode msrls CPSR_fsxc, #256" in {
        MoveToStatusRegister(1.toByte, 24.toByte, CPSR, Fields.extension + Fields.control + Fields.status + Fields.flags, Condition.LowerOrSame).encode should be(Hex.MSB("932ffc01"))
      }
    }
  }
}
