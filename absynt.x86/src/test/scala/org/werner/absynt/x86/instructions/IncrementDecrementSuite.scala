  ...
  "an Increment instruction" when {
    "in real mode" should {
      ...
      "correctly encode inc BYTE PTR [bp+di]" in {
        Increment(Pointer.word[ByteSize](BP+DI)).encodeByte should be(Hex.lsb("FE 03"))
      }
      "correctly represent inc BYTE PTR [bp+di] as a string" in {
        Increment(Pointer.word[ByteSize](BP+DI)).toString should be("inc BYTE PTR [bp+di]")
      }
      ...
      "correctly encode inc WORD PTR [bp+di]" in {
        Increment(Pointer.word[WordSize](BP+DI)).encodeByte should be(Hex.lsb("FF 03"))
      }
      "correctly represent inc WORD PTR [bp+di] as a string" in {
        Increment(Pointer.word[WordSize](BP+DI)).toString should be("inc WORD PTR [bp+di]")
      }
