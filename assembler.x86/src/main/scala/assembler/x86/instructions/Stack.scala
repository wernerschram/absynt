...
object Push {
  ...
  def apply(register: WideRegister)(implicit label: Label, processorMode: ProcessorMode) =
    R16(register)
  ...
  def apply(operand: ModRMEncodableOperand with FixedSizeOperand)(implicit label: Label, processorMode: ProcessorMode) =
    RM16(operand)
