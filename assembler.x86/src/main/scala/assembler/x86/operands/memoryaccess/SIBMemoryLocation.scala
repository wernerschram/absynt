...
sealed class SIBMemoryLocation(val index: SIBIndexRegister, val base: SIBBaseRegister, displacement: Seq[Byte], val scale: Int,
...
object SIBMemoryLocation {
  ...
  def apply(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte] = Seq.empty[Byte], scale: Int = 1) =
    new SIBMemoryLocation(index, base, displacement, scale, index.defaultSIBSegment)

  def byteSize(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte] = Seq.empty[Byte], scale: Int = 1) =
    FixedSizeSIBMemoryLocation(index, base, displacement, scale, ValueSize.Byte, index.defaultSIBSegment)

  def wordSize(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte] = Seq.empty[Byte], scale: Int = 1) =
    FixedSizeSIBMemoryLocation(index, base, displacement, scale, ValueSize.Word, index.defaultSIBSegment)

  def doubleWordSize(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte] = Seq.empty[Byte], scale: Int = 1) =
    FixedSizeSIBMemoryLocation(index, base, displacement, scale, ValueSize.DoubleWord, index.defaultSIBSegment)

  def quadWordSize(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte] = Seq.empty[Byte], scale: Int = 1) =
    FixedSizeSIBMemoryLocation(index, base, displacement, scale, ValueSize.QuadWord, index.defaultSIBSegment)

  final class FixedSizeSIBMemoryLocation private(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte], scale: Int,
  ...
  private object FixedSizeSIBMemoryLocation {
    def apply(index: SIBIndexRegister, base: SIBBaseRegister, displacement: Seq[Byte], scale: Int, operandByteSize: OperandSize,
              segment: SegmentRegister) =
